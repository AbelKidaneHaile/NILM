...
from sklearn.metrics import classification_report, multilabel_confusion_matrix, accuracy_score, f1_score, recall_score, average_precision_score
...
class SmartMeterDataset(Dataset):
    ...
    def __init__(self, csv_path, scaler=None, is_train=True):
        ...
        self.feature_cols = ['powerallphases', 'powerl1', 'powerl2', 'powerl3', 
        ...
        self.target_cols = ['Fridge', 'Dryer', 'Coffee machine', 'Kettle', 
...
class SmartMeterClassifier(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dims=[512, 256, 128], output_dim=7, dropout=0.3):
...
class MetricsTracker:
    ...
    def update(self, train_loss, val_loss, train_acc, val_acc, 
               train_f1, val_f1, train_recall, val_recall, train_map, val_map):
...
def plot_training_metrics(metrics_tracker, save_dir="./"):
    ...
    # Accuracy
    axes[0, 1].plot(epochs, metrics_tracker.train_accuracies, 'b-', label='Training Accuracy', linewidth=2)
    ...
    # F1 Score
    axes[0, 2].plot(epochs, metrics_tracker.train_f1_scores, 'b-', label='Training F1', linewidth=2)
    axes[0, 2].plot(epochs, metrics_tracker.val_f1_scores, 'r-', label='Validation F1', linewidth=2)
    ...
    # Recall
    axes[1, 0].plot(epochs, metrics_tracker.train_recall_scores, 'b-', label='Training Recall', linewidth=2)
    axes[1, 0].plot(epochs, metrics_tracker.val_recall_scores, 'r-', label='Validation Recall', linewidth=2)
    ...
    # mAP
    axes[1, 1].plot(epochs, metrics_tracker.train_map_scores, 'b-', label='Training mAP', linewidth=2)
    axes[1, 1].plot(epochs, metrics_tracker.val_map_scores, 'r-', label='Validation mAP', linewidth=2)
    ...
    # Combined metrics
    axes[1, 2].plot(epochs, metrics_tracker.val_accuracies, 'r-', label='Accuracy', linewidth=2)
    axes[1, 2].plot(epochs, metrics_tracker.val_f1_scores, 'g-', label='F1 Score', linewidth=2)
    axes[1, 2].plot(epochs, metrics_tracker.val_recall_scores, 'b-', label='Recall', linewidth=2)
    axes[1, 2].plot(epochs, metrics_tracker.val_map_scores, 'm-', label='mAP', linewidth=2)
...
def generate_classification_report(y_true, y_pred, target_names, save_dir="./"):
    ...
    report = classification_report(y_true, y_pred_binary, target_names=target_names, 
    ...
    plt.suptitle('Confusion Matrices for Each Appliance', fontsize=16, fontweight='bold')
    plt.tight_layout()
...
def main():
    ...
    for epoch in range(config['epochs']):
        ...
        # Update metrics
        metrics_tracker.update(train_loss, val_loss, train_acc, val_acc,
    ...
    _, test_acc, test_f1, test_recall, test_map, test_predictions, test_targets = validate_epoch(
        model, test_loader, criterion, device
    )
    
    print(f"Test Results - Acc: {test_acc:.4f}, F1: {test_f1:.4f}, "
    ...
    report = generate_classification_report(test_targets, test_predictions, 
                                          target_names, config['save_dir'])
